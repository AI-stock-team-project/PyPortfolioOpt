...
# 모멘텀 1 mo - sharpe / eff 랑 같은 csv 씀.
...
def Momentum1mo_eff():
    ...
    # assets = np.array(momentum_1month_rank.index[:30]) #pd.read_csv('momentum_1mo_assets.csv')
    ...
    df = pd.DataFrame()
    ##################### 여기 추가됐습니다 : 관리종목 제거 ######################
    # temp_assets = pd.read_csv('momentum_1mo_assets.csv')
